...
/* ═══════════════════════════════════════════════════════════════
 * SHOR — Period-finding via room intermodulation.
 *
 * Shor's algorithm: factor N by finding the period r of a^x mod N
 * using the room's R820T2 mixer as the Fourier-transform oracle.
 *
 * For each k in [0, D_room): encode a^k mod N as bin amplitude.
 * The mixer computes f_i ± f_j for all pairs simultaneously,
 * creating constructive interference at the true period r.
 * Then gcd(a^(r/2) ± 1, N) gives the factors.
 *
 *   SHOR N [a] [banks]   — factor N, optionally specify base a
 *   SHOR 15              — factor 15 (3×5)
 *   SHOR 21              — factor 21 (3×7)
 *   SHOR 77 3 8          — factor 77 with a=3, 8 banks
 *
 * With BANKS pre-configured, uses full D_room. Otherwise
 * auto-configures banks based on N.
 * ═══════════════════════════════════════════════════════════════ */
static int op_shor(QvmCtx *q, double a1, double a2){
    unsigned long long N = (unsigned long long)a1;
    unsigned long long force_a = (unsigned long long)a2;
    if (N < 4) { printf("  [SHOR] N must be >= 4\n"); return 0; }

    /* Auto-configure banks if not already set */
    int orig_banks = q->num_banks;
    int room_dim, bdim = q->bank_dim;
    if (orig_banks <= 1) {
        int nb = N < 64 ? 2 : (N < 256 ? 4 : 8);
        if (nb > MAX_BANKS) nb = MAX_BANKS;
        char cmd[64]; snprintf(cmd, 64, "BANKS %d", nb);
        qvm_eval(q, cmd);
    }
    room_dim = q->num_banks * bdim;

    printf("  ╔══════════════════════════════════════════════════════════╗\n");
    printf("  ║  SHOR'S ALGORITHM — room mixer as period oracle          ║\n");
    printf("  ║  N = %-6llu  |  D_room = %d  |  %d banks                ║\n",
           N, room_dim, q->num_banks);
    printf("  ╚══════════════════════════════════════════════════════════╝\n\n");

    if (!q->sdr_ok) {
        printf("  [SHOR] No SDR hardware — cannot run period oracle.\n");
        goto restore;
    }

    srand(time(NULL));
    double *seq = malloc(room_dim * sizeof(double));
    if (!seq) { printf("  [SHOR] OOM\n"); goto restore; }

    int attempts = force_a ? 1 : 15;
    for (int attempt = 0; attempt < attempts; attempt++) {
        unsigned long long a = force_a ? force_a
            : (2 + (rand() % (N - 3)));
        if (a < 2 || a >= N) continue;

        /* Quick GCD check — trivial factor */
        unsigned long long g = a;
        { unsigned long long x = N;
          while (x) { unsigned long long t = x; x = g % x; g = t; }
        }
        if (g > 1 && g < N) {
            printf("  [SHOR] a=%llu  gcd(a,N)=%llu → trivial factor!\n"
                   "         N = %llu × %llu\n", a, g, g, N/g);
            free(seq); goto restore;
        }

        /* Compute a^k mod N for k = 0..room_dim-1 */
        unsigned long long val = 1;
        double maxval = 0;
        for (int k = 0; k < room_dim; k++) {
            seq[k] = (double)val;
            if (seq[k] > maxval) maxval = seq[k];
            val = (val * a) % N;
        }

        printf("  [SHOR] attempt %d  a=%llu  sequence (max=%.0f)\n",
               attempt, a, maxval);

        /* OFDM per-bank: encode seq subset, TX simultaneously via DMA,
           capture intermodulation products from the room mixer. */
        uint32_t save_freq = q->sdr->freq;
        double best_pwr = 0;
        int best_k = 0;
        ...
        for (int b = 0; b < q->num_banks; b++) {
            ...
            int bank_d = bdim < q->wf.d ? bdim : q->wf.d;

            /* Encode anti-sym pairs for this bank */
            memset(q->wf.re,  0, q->wf.d * sizeof(double));
            memset(q->wf.im,  0, q->wf.d * sizeof(double));
            for (int k = 0; k < bank_d; k++) {
                int idx = bank_start + k;
                double amp = maxval > 0 ? seq[idx] / maxval : 0;
                q->wf.re[k] = +amp;
                q->wf.im[k] =  0;
                if (k > 0 && bank_d > k && bank_d - k < bank_d) {
                    q->wf.re[bank_d - k] -= amp * 0.5;
                }
            }
            for (int k = 0; k < bank_d; k++)
                q->wf.prob[k] = q->wf.re[k]*q->wf.re[k] + q->wf.im[k]*q->wf.im[k];
            for (int i = 0; i < 16 && i < bank_d; i++)
                q->wf.re[i] = q->wf.im[i] = q->wf.prob[i] = 0;

            /* TX via OFDM at this bank's center frequency */
            uint32_t bfreq = (uint32_t)q->freq + (uint32_t)(b * q->rate);
            sdr_retune(q->sdr, bfreq);
            usleep(5000);
            gate_ofdm_tx(q->sdr, &q->wf, NULL);
            ...
            { struct v4l2_buffer vb; memset(&vb,0,sizeof(vb));
              ...
              if(ioctl(q->sdr->fd,VIDIOC_DQBUF,&vb)==0)ioctl(q->sdr->fd,VIDIOC_QBUF,&vb); }
        }

        /* Wait for room multipath to settle — mixer produces IM2 products */
        usleep(80000);

        /* Capture each bank, find max-power bin */
        for (int b = 0; b < q->num_banks; b++) {
            uint32_t bfreq = (uint32_t)q->freq + (uint32_t)(b * q->rate);
            sdr_retune(q->sdr, bfreq);
            usleep(8000);
            ...
            for (int k = 2; k < q->wf.d; k++) {
                int idx = b * bdim + k;
                double p = q->wf.prob[k];
                if (p > best_pwr) { best_pwr = p; best_k = idx; }
            }
        }
        ...
        unsigned long long r = best_k;
        printf("  [SHOR] period candidate r = %llu (pwr=%.6f at bin %d)\n",
               r, best_pwr, best_k);
        if (r < 2) { printf("  [SHOR] r too small, retry\n"); continue; }
        if (r % 2 != 0) r *= 2;

        unsigned long long half_exp = 1;
        { unsigned long long e = r/2, base = a % N;
          while (e) { if (e&1) half_exp = (half_exp*base)%N; e>>=1; base = (base*base)%N; } }

        unsigned long long f = half_exp + 1, y = f, x = N;
        while (y) { unsigned long long t = y; y = x % y; x = t; } f = x;
        if (f > 1 && f < N) {
            printf("  ★★★ FACTORED ★★★  N=%llu = %llu × %llu\n", N, f, N/f);
            printf("         a=%llu  r=%llu  a^(r/2)=%llu\n", a, r, half_exp);
            free(seq); goto restore;
        }

        { unsigned long long cand = half_exp ? half_exp-1 : N-1;
          y = cand; x = N; while (y) { unsigned long long t = y; y = x % y; x = t; } f = x; }
        if (f > 1 && f < N) {
            printf("  ★★★ FACTORED ★★★  N=%llu = %llu × %llu\n", N, f, N/f);
            printf("         a=%llu  r=%llu  a^(r/2)=%llu\n", a, r, half_exp);
            free(seq); goto restore;
        ...
        printf("  [SHOR] gcd(a^(r/2)±1,N) trivial — retry\n");
    }

    printf("  [SHOR] %d attempts, no factor.\n", attempts);
    free(seq);

restore:
    if (orig_banks <= 1 && q->num_banks > 1) {
        char cmd[32]; snprintf(cmd, 32, "BANKS 1"); qvm_eval(q, cmd);
    }
    return 0;
}
static void qvm_init_ops(QvmCtx *q){
    ...
    qvm_reg(q, "SHOR",      op_shor,      "Shor's factorisation: period-finding via room mixer");
}
